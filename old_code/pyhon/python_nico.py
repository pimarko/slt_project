	...
	for ii in range(voxel_num):
		for jj in range(ii+1,voxel_num):
			if(is_neighbour(neighbours_matrix,ii,jj)):
				if(DISIMILARITY_MEASURE_INNER_PRODUCT):
					D[ii,jj] = np.square(np.dot(data_matrix[ii,3:data_matrix.shape[1]-1],data_matrix[jj,3:data_matrix.shape[1]-1]))
					D_normal[ii,jj] = np.dot(data_matrix[ii,3:data_matrix.shape[1]-1],data_matrix[jj,3:data_matrix.shape[1]-1])
					nnz = nnz + 1
				else:
